...
export default function ChatInterface() {
  ...
  const [selectedUser, setSelectedUser] = React.useState<ChatUser | null>(null);
  const { user } = useAuth();
  const { messages, setActiveRoom, activeRoomId } = useChatStore();

  const handleUserSelect = React.useCallback(
    (user: ChatUser) => {
      const roomId = `${user.id}-${user.username}`;
      setActiveRoom(roomId);
      setSelectedUser(user);
    },
    [setActiveRoom],
  ...
  const handleSendMessage = React.useCallback(
    async (e: React.FormEvent) => {
      e.preventDefault();
      if (!selectedUser || !messageInput.trim() || !user) return;
    ...
    [selectedUser, messageInput, user],
  );

  const handleComplete = React.useCallback(() => {
    if (selectedUser) {
      useChatStore.getState().clearMessages(activeRoomId!);
      setSelectedUser(null);
    }
  ...
  React.useEffect(() => {
    ...
    ChatService.initializeSession(selectedUser.id);
